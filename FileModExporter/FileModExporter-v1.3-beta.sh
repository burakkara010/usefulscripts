#!/bin/bash
echo -e "\033[1;32m"
echo "script: export-folder-file-last-modified-v1.3.sh"
echo -e "\033[0m"
echo -e "\033[1;33m"
...
sheet_limit = 1048576  # Maximum rows per sheet

current_sheet_index = 1
worksheet_all = workbook.add_worksheet(f"All_Files_1")
...
for root, dirs, files in os.walk("."):
    ...
    if files:
        ...
        for file in progress_bar:
            ...
            if row_all == sheet_limit:
                print(f"Row limit reached for sheet All_Files_{current_sheet_index}. Creating new sheet...")
                current_sheet_index += 1
                worksheet_all = workbook.add_worksheet(f"All_Files_{current_sheet_index}")
                worksheet_all.write(0, 0, "Folder")
                worksheet_all.write(0, 1, "Filename")
                worksheet_all.write(0, 2, "File Size")
                worksheet_all.write(0, 3, "Last Modified Date")
