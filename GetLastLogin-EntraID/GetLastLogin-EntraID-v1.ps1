...
function Connect-Graph {
    ...
    function Get-ConnectMgGraphExtraParams {
        ...
        if ($connectCmd -and $connectCmd.Parameters.ContainsKey('ForceRefresh')) {
            # Ensure we don't reuse an older cached token missing newly-activated roles/scopes.
            $extra['ForceRefresh'] = $true
        }
        if ($connectCmd -and $connectCmd.Parameters.ContainsKey('ContextScope')) {
            # Keep auth state scoped to this process to reduce surprises from previous sessions.
    ...
    function Convert-SecureStringToPlainText {
        ...
        # Safe conversion pattern commonly used in PowerShell.
        ...
        try {
            ...
            if ($azCtx2) {
                ...
                if ($tokenResponse -and $tokenResponse.Token) {
                    ...
                    # Validate the token can actually call Graph.
                    # (Connect-MgGraph with -AccessToken does not always validate upfront.)
                    try {
    ...
    function Test-CanListUsers {
        try {
            ...
            # Treat other errors as non-blocking here.
            return $true
        }
    }

    function Test-CanReadSignInActivityForAnyUser {
        # This is a stronger check than reading /users/{me} because Excel lookups target other users.
        # Prefer beta for signInActivity support.
        $urisToTry = @(
    ...
    # Ensure we actually have the scopes required for this script.
    ...
    # Preflight permissions required for the Excel run (which reads other users).
...
try {
    if ($Menu) {
        ...
        # Default behavior: run the full workflow in one go.
